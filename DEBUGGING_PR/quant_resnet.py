...
def evaluate(model, device_: str):
    ...
    if not (device_ in['cpu', 'cuda']):
        raise NotImplementedError('Provide device_')

    input_batch = input_batch.to(device_)
    model.to(device_)
    if device_ == 'cuda':
